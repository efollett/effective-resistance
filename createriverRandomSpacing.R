#creates 1D river channel with randomly distributed jams
createriverRandomSpacing <-function(HBF,S,BBF,CA,Ls,Cf0,Beffi){
g=9.8
...
while (resid>0.05){
 # spacing=rexp(N,1/Ls)
  spacing=rtruncexp(N,rate=1/Ls,a=3,b=Inf)
  resid=abs(mean(spacing)-Ls)/Ls
}

LR = sum(spacing)+Ls; #because Nsegs=Lx-2 in model
Nsegs = N
slope=S
...
riv["x"] <- list(x=c(0,cumsum(spacing),LR)) 
...
riv["Beff"]<-list(Beff=rep(1,length(riv$x))) #Beff*BBF=channel width above bankfull depth; Beff=1 has no "floodplain"
